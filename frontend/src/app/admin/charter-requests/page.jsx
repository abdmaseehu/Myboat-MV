...
export default function CharterRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const res = await api.get("/charter-requests");
      setRequests(res.data?.data || []);
    } catch (e) {
      toast.error(e.message || "Failed to load charter requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      const passengerNames = createForm.passengerNamesText
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean);
      const payload = {
        ...createForm,
        passengers: Number(createForm.passengers) || 1,
        passengerNames,
        isManual: true,
        status: "ACCEPTED",
        vesselId: createForm.vesselId || null,
        arrivalFlightTime: createForm.arrivalFlightTime || null,
        departureFlightTime: createForm.departureFlightTime || null,
        departureTime: createForm.departureTime || null,
      };
      delete payload.passengerNamesText;
      await api.post("/charter-requests", payload);
      toast.success("Manual charter created & confirmation queued");
      setCreateOpen(false);
      fetchRequests();
    } catch (e) {
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Review and send quotations for private charter requests
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-10 text-center text-sm text-muted-foreground">
            <Anchor className="h-8 w-8 mx-auto mb-2 text-sky-500/70" />
            No charter requests here yet.
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4 flex flex-col md:flex-row md:items-center gap-3">
                <div className="flex-1 min-w-0 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    ...
                    {r.departureTime && (
                      <span className="inline-flex items-center gap-1">
                        <Clock className="h-3.5 w-3.5" /> {fmtTime(r.departureTime)}
                      </span>
                    )}
                    <span className="inline-flex items-center gap-1">
                      <Users className="h-3.5 w-3.5" /> {r.passengers} passengers
                    </span>
                    {r.returnTrip && (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    {fmtTime(viewRequest.departureTime) || "Flexible"}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground text-xs">Passengers</div>
                  <div className="font-medium">{viewRequest.passengers}</div>
                </div>
                <div>
                  <div className="text-muted-foreground text-xs">Return trip</div>
                  <div className="font-medium">
                    {viewRequest.returnTrip
                      ? `Yes — ${fmtDate(viewRequest.returnDate)} ${
                          fmtTime(viewRequest.returnTime) || ""
                        }`
                      : "No"}
                  </div>
                </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Create Manual Charter Request</DialogTitle>
            <DialogDescription>
              Create a confirmed private charter on behalf of a guest. A confirmation
              email will be sent to the guest and to your operator inbox, and an
              e-ticket will be generated.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <Section title="Trip Details">
              ...
              <Field label="Departure Time">
                <Input
                  ...
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, departureTime: e.target.value }))
                  }
                />
              </Field>
              <Field label="Passengers" required>
                <Input
                  ...
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, passengers: e.target.value }))
                  }
                />
              </Field>
              <Field label="Vessel">
              ...
              <Field label="Captain Phone">
                <Input
                  value={createForm.captainPhone}
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, captainPhone: e.target.value }))
                  }
                />
              </Field>
              <Field
                label="Meeting Point"
                hint="Where the guest should meet for boarding. Shown on the e-ticket."
              >
                <Input
                  value={createForm.meetingPoint}
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, meetingPoint: e.target.value }))
                  }
                />
              </Field>
            </Section>
            ...
            <Section title="Flight Details">
              <Field label="Arrival Flight Number">
                <Input
                  value={createForm.arrivalFlight}
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, arrivalFlight: e.target.value }))
                  }
                />
              </Field>
              <Field label="Arrival Flight Time">
                <Input
                  type="datetime-local"
                  value={createForm.arrivalFlightTime}
                  onChange={(e) =>
                    setCreateForm((s) => ({
                      ...s,
                      arrivalFlightTime: e.target.value,
                    }))
                  }
                />
              </Field>
              <Field label="Departure Flight Number">
                <Input
                  value={createForm.departureFlight}
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, departureFlight: e.target.value }))
                  }
                />
              </Field>
              <Field label="Departure Flight Time">
                <Input
                  type="datetime-local"
                  value={createForm.departureFlightTime}
                  onChange={(e) =>
                    setCreateForm((s) => ({
                      ...s,
                      departureFlightTime: e.target.value,
                    }))
                  }
                />
              </Field>
              <Field
                label="Hotel Name"
                hint="Used on the e-ticket when no arrival flight is provided."
              >
                <Input
                  value={createForm.hotelName}
                  onChange={(e) =>
                    setCreateForm((s) => ({ ...s, hotelName: e.target.value }))
                  }
                />
              </Field>
            </Section>
            ...
            <Section title="Passengers">
              <Field
                ...
                hint="One name per line. Shown on the e-ticket."
                full
              >
                <Textarea
                  rows={4}
                  value={createForm.passengerNamesText}
                  onChange={(e) =>
                    setCreateForm((s) => ({
                      ...s,
                      passengerNamesText: e.target.value,
                    }))
                  }
                />
              </Field>
            ...
            <Section title="Pricing & Payment">
              ...
              <Field label="Payment Type">
                <Select
                  ...
                  onValueChange={(v) =>
                    setCreateForm((s) => ({ ...s, paymentType: v }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <p className="col-span-full text-xs text-muted-foreground">
                MVR amounts cannot be paid online via card. The guest will receive a
                confirmation email; collect payment via the selected payment method.
              </p>
            </Section>
