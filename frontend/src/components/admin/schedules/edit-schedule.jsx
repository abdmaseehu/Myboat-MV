...
export default function EditSchedule({ schedule, open, onClose, onSuccess }) {
  const [loading, setLoading] = useState(false);
  const [routes, setRoutes] = useState([]);
  const [vehicles, setVehicles] = useState([]);
  const [activeTab, setActiveTab] = useState("basic");
  const [daySpecificTiming, setDaySpecificTiming] = useState({});
  ...
  useEffect(() => {
    if (schedule) {
      form.reset({
        ...
        vehicleId:
          schedule.vehicleId ||
          (schedule.vehicles && schedule.vehicles[0]?.id) ||
          "",
        busType: schedule.busType || "AC_SLEEPER",
        status: schedule.status || "ACTIVE",
        isActive: schedule.isActive ?? true,
        availableSeats: schedule.availableSeats ?? 0,
        blockedSeats: schedule.blockedSeats ?? 0,
        priceLocalMvr: schedule.priceLocalMvr ?? undefined,
        priceExpatMvr: schedule.priceExpatMvr ?? undefined,
        priceTouristUsd: schedule.priceTouristUsd ?? undefined,
        // A schedule written before age bands existed has the column defaults.
        childFareEnabled: schedule.childFareEnabled !== false,
        childPriceLocalMvr: schedule.childPriceLocalMvr ?? undefined,
        childPriceExpatMvr: schedule.childPriceExpatMvr ?? undefined,
        childPriceTouristUsd: schedule.childPriceTouristUsd ?? undefined,
        childPercent: schedule.childPercent ?? 50,
        infantPercent: schedule.infantPercent ?? 0,
        departureDate: formatDateForInput(schedule.departureDate),
        departureTime: formatTimeForInput(schedule.departureTime),
        arrivalDate: formatDateForInput(schedule.arrivalDate),
        arrivalTime: formatTimeForInput(schedule.arrivalTime),
        isRecurring: !!schedule.isRecurring,
        daysOfWeek: schedule.daysOfWeek || [],
        recurrenceEndDate: formatDateForInput(schedule.recurrenceEndDate),
      });

      // Rehydrate day-specific timing
      const dst = {};
      if (schedule.daySpecificTiming) {
        Object.entries(schedule.daySpecificTiming).forEach(([d, cfg]) => {
          dst[d] = {
            enabled: true,
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const arr = values.isRecurring
        ...
        : new Date(
            `${values.arrivalDate || values.departureDate}T${values.arrivalTime}`
          );
      ...
      const response = await api.put(`/bus-schedules/${schedule.id}`, payload);
                        ...
                        <SelectContent>
                          {routes.map((r) => (
                            <SelectItem key={r.id} value={r.id}>
                              {r.sourceCity} to {r.destinationCity}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
              ...
              <TabsContent value="pricing" className="space-y-4 mt-4">
                <div className="border rounded-lg p-4 space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Set different prices for Local, Expat, and Tourist
                      passengers
                    </p>
                  </div>
