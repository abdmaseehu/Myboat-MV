...
export default function EditDriver({ driver, open, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (driver) {
      setFormData({
        ...
        drivingStatus: driver.drivingStatus || "AVAILABLE",
      });
      ...
      setPreviews({
        ...
        driverLicenseBack: driver.driverLicenseBack
          ? `${process.env.NEXT_PUBLIC_ROOT_URL}${driver.driverLicenseBack}`
          : null,
        driverLicenseFront: driver.driverLicenseFront
          ? `${process.env.NEXT_PUBLIC_ROOT_URL}${driver.driverLicenseFront}`
          : null,
      });
    }
  }, [driver]);
  ...
  const clearFilePreview = (name) => {
    ...
    setPreviews((prev) => ({
      ...prev,
      [name]: driver[name]
        ? `${process.env.NEXT_PUBLIC_ROOT_URL}${driver[name]}`
        : null,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Submit to API
      await api.patch(`/drivers/${driver.id}`, formDataToSend, {
        headers: {
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col items-center gap-4">
                {previews.driverPhoto ? (
                  ...
                  <label className="h-32 w-32 rounded-lg border-2 border-dashed border-muted-foreground/20 flex flex-col items-center justify-center gap-1 cursor-pointer hover:border-sky-500/50 transition-colors">
